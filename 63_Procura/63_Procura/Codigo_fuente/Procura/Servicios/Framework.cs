...
namespace WINlogistica
{
    ...
    {
        public string NombreArchivo(string strArchivoDestino)
        {
        ...
        public void DownLoadArchivo(string strNombreArchivo)
        {
            ftp ftpClient = new ftp("ftp://ftpsafnet.amsac.pe/", @"activosmineros\admin.app1", "Amsac..2023");
            string strCarpeta = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            string strArchivoDesCarga = strNombreArchivo.TrimEnd();
            ftpClient.download(strArchivoDesCarga, strArchivoDesCarga);
            string strArchivo = strArchivoDesCarga;


            try
            {
                var proceso = new System.Diagnostics.Process();
                proceso.StartInfo.FileName = Convert.ToString(strArchivo);
                proceso.Start();
            }
            catch (Exception ex)
            {
