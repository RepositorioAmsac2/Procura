...
{
    public partial class UC_Lista_Parametro : DevExpress.XtraEditors.XtraUserControl
    {
        private Model.ParametroLogistica parametropendiente = new Model.ParametroLogistica();
        //private SRsafnet.SAFnetEClient objWCF = new SRsafnet.SAFnetEClient();
        private Service.Data objDATA = new Service.Data();
        public UC_Lista_Parametro()
        {
            InitializeComponent();
            this.lista();
            tileView1.FocusedRowHandle = 0;
            this.Grd_ListaParametro.SizeChanged += (s, e) => { CompactTemplate = Grd_ListaParametro.Width <= 300; };
            //if (MainFormHelper.TakeScreens) tileView1.AnimateArrival = false;
            UpdateColors();
        }

        Color UnreadTextColor;
        private static UC_Lista_Parametro _instance;
        public static UC_Lista_Parametro Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new UC_Lista_Parametro();
                return _instance;
            }
        }
        ...
        private void lista()
        {
            ...
            {
                //jesusdt = objWCF.Lista_ParametroFormulacion(MyStuff.CodigoEmpresa).Tables[0];

            }
            else
            {
                dt = objDATA.Lista_ParametroLogistica(MyStuff.CodigoEmpresa).Tables[0];
            }
            this.Grd_ListaParametro.DataSource = dt;
        ...
        {
            if (string.IsNullOrEmpty(Convert.ToString(this.tileView1.GetFocusedRowCellValue("cCodParametroLogistica"))) == false)
            {
                //int intTipoparametro = Convert.ToInt32(this.tileView1.GetFocusedRowCellValue("Itipoparametro"));
                string strCodParametro = Convert.ToString(this.tileView1.GetFocusedRowCellValue("cCodParametroLogistica"));
                ...
                Frm_ActualizarParametro frm = new Frm_ActualizarParametro();
                ...
                {
                    parametropendiente = frm.parametroanterior;
                }

                if (frm.blnGrabar == true)
                {
                    parametropendiente = (new Model.ParametroLogistica());
                    this.ActualizaDatosGrid(frm.parametroanterior);
        ...
        private void ActualizaDatosGrid(Model.ParametroLogistica obj)
        {
            Type t = obj.GetType();
        ...
        {
            string parametroCodigo = Convert.ToString(this.tileView1.GetFocusedRowCellValue("CcodCeCo"));

            XtraMessageBoxArgs argumentos = new XtraMessageBoxArgs();
            argumentos.Caption = "Advertencia";
            argumentos.Text = "Desea eliminar el Parametro " + parametroCodigo.TrimEnd() + " ?";
            ...
            {
                int Id = Convert.ToInt32(this.tileView1.GetFocusedRowCellValue("iIdParametroLogistica"));

                if (MyStuff.UsaWCF)
                {
                    //jesusobjWCF.Elimina_ParametroFormulacion(Id);
                }
                else
                    objDATA.Elimina_ParametroLogistica(Id);
        ...
        {
            this.ShowGridPreview(Grd_ListaParametro);
        }
