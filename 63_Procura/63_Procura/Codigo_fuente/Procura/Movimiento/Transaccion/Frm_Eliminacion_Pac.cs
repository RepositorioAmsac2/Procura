...
{
    public partial class Frm_Eliminacion_Pac : Form
    {
        ...
        public void ShowMe(string strNumProcedimiento)
        {
            ...
            if (MPC.iIdProcesoLogistico_Cabecera != 0)
            {
                ...
                this.MostrarNumProcedimiento();
            }
        }

        private void MostrarNumProcedimiento()
        {
            if (MyStuff.UsaWCF == true)
            {
                MPD = objWCF.Recupera_Pac_Detalle_Codigo(Convert.ToString(this.Txt_NumProcedimiento.Text));
            }
            else
            {
            ...
            this.tsb_Exluir_Salir.Enabled = true;
        ...
        private bool Excluir_PAC()
        {
            bool blnSalir = false;

            Frm_Eliminacion_Pac_D frm = new Frm_Eliminacion_Pac_D();

            frm.ShowMe();
            if (frm.blnGrabar == true)
            {

                MPD.cCodMotivoEliminacion = frm.strCodMotivoEliminacion;
                MPD.dFecEliminado = frm.dateFecEliminacion;
                MPD.tMotEliminado = frm.strNota;
                ...
                bool blnElimina = false;
                if (MyStuff.UsaWCF == true)
                {
                    blnElimina = objWCF.Elimina_Pac_Detalle(MPD);
                }
                else
                {
                    blnElimina = sdl.Elimina_ProcesoLogistico_Detalle(MPD);
                }
                ...
                if (blnElimina != false)
                {
                   this.tsb_Exluir_Salir.Enabled = false;
                    blnGrabo = true;
                    MessageBox.Show("OK: El registro se Excluyo Correctamente");
                    blnSalir = true;
                }
                else
                {
                    MessageBox.Show("Error: No se pudo Excluir el registro");
                }
        ...
        private void Txt_CodCentro_Leave(object sender, EventArgs e)
        {
            ...
            if (MyStuff.UsaWCF == true)
            {
                MC = objWCF.Recupera_Centro_Codigo(strCodCentro);
            }
            else
            {
        ...
        private void tsb_Excluir_Salir_Click(object sender, EventArgs e)
        {
            if (this.VerificaIngresoMovimiento())
            {
                if( this.Excluir_PAC() == true )
                {
                    this.Close();
                }
