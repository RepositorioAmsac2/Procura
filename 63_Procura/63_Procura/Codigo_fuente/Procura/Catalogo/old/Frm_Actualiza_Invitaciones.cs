...
namespace WINlogistica
{
    public partial class Frm_Actualiza_Invitaciones : Form
    {
        ...
        Service.DataLogistica sdl = new Service.DataLogistica();
        ...
        private string strCodPasoSiguiente = "";
                          ...
                          )
        {
            ...
            strCodPasoSiguiente = cCodPasoSiguiente;
            ...
            if (intEliminaRegistro == DialogResult.Yes)
            {
                ...
                else
                {
                    ...
                    if (Grd_Cotizacion.Rows.Count > 0)
                    {
                        ...
                        intIdCotizacion = Convert.ToInt32(oRow.Cells[1].Value);
                        this.verDatosCotizacion(intIdCotizacion);
        ...
        private void AperturaNuevoPaso()
        {
            ...
            if (strCodPasoSiguiente != "")
            {
                string strCodPaso_Value = strCodPasoSiguiente;
                if (MyStuff.UsaWCF == true)
                {
                }
                else
                {
                    MP = sdl.Recupera_Paso_codigo(MPD.cCodTipoPlantilla,
                                                    strCodPasoSiguiente
                                                 );
                }
                ...
                if (MyStuff.UsaWCF == true)
                {
                    dateFechas = objWCF.Recupera_Pac_Paso_Apertura_Proceso(MPD.cNumProcedimiento, strCodPasoSiguiente);
                }
                else
                {
                    dateFechas = sdl.Recupera_ProcesoLogistico_Paso_Apertura_Proceso(MPD.cNumProcedimiento, MPD.cCodTipoPlantilla, strCodPasoSiguiente);
                }
                ...
                MPDP.cCodPaso = strCodPasoSiguiente;
